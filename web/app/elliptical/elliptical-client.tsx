...
interface EllipticalClientProps {
  sessions: EllipticalSession[]
}
...
function kcalPerMin(session: EllipticalSession): number | null {
...
function intensityColor(kpm: number): string {
  if (kpm < 6) return '#22c55e'   // green
  if (kpm < 9) return '#eab308'   // yellow
  if (kpm < 12) return '#f97316'  // orange
  return '#ef4444'                 // red
}

export function EllipticalClient({ sessions }: EllipticalClientProps) {
  if (sessions.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center space-y-3">
        <span className="text-5xl">〇</span>
        <h2 className="text-lg font-semibold text-text-primary">No elliptical data yet</h2>
        <p className="text-sm text-text-secondary max-w-xs">
          Sync your iPhone to import elliptical workouts from Apple Health. Start an Elliptical
  ...
  // ─── Weekly duration chart (last 13 weeks) ──────────────────────────────
  const weeklyDuration = (() => {
    ...
    for (const s of sessions) {
      ...
      if (weeks.has(label)) {
        weeks.set(label, (weeks.get(label) ?? 0) + s.duration_minutes)
      }
    }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-surface rounded-2xl border border-border p-4">
        <h3 className="text-sm font-medium text-text-secondary mb-3">
          Weekly Duration (min) — Last 13 Weeks
        </h3>
        <ResponsiveContainer width="100%" height={150}>
          <BarChart data={weeklyDuration} margin={{ top: 4, right: 4, left: -4, bottom: 0 }}>
            <CartesianGrid
              strokeDasharray="3 3"
            ...
            />
            <Bar dataKey="minutes" fill={CYAN} radius={[3, 3, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
        ...
        <div className="bg-surface rounded-2xl border border-border p-4">
          ...
          <div className="flex items-center justify-center gap-4 mt-2 flex-wrap">
            <span className="flex items-center gap-1 text-xs text-text-secondary">
              <span className="inline-block w-2.5 h-2.5 rounded-full bg-green-500" /> &lt;6 kcal/min
            </span>
            <span className="flex items-center gap-1 text-xs text-text-secondary">
              <span className="inline-block w-2.5 h-2.5 rounded-full bg-yellow-500" /> 6–9
            </span>
            <span className="flex items-center gap-1 text-xs text-text-secondary">
              <span className="inline-block w-2.5 h-2.5 rounded-full bg-orange-500" /> 9–12
            </span>
            <span className="flex items-center gap-1 text-xs text-text-secondary">
              <span className="inline-block w-2.5 h-2.5 rounded-full bg-red-500" /> &gt;12
            </span>
          </div>
        </div>
      )}
              ...
              {recentSessions.map((s, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 text-right tabular-nums font-medium" style={{ color: CYAN }}>
