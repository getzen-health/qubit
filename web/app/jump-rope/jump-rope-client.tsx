...
interface JumpRopeClientProps {
  sessions: JumpRopeSession[]
}

const CYAN = '#06b6d4'
const CYAN_DIM = 'rgba(6,182,212,0.4)'
...
function kcalPerMin(session: JumpRopeSession): number | null {
...
export function JumpRopeClient({ sessions }: JumpRopeClientProps) {
  if (sessions.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center space-y-3">
        <span className="text-5xl">🪢</span>
        <h2 className="text-lg font-semibold text-text-primary">No jump rope data yet</h2>
        <p className="text-sm text-text-secondary max-w-xs">
          Sync your iPhone to import jump rope workouts from Apple Health. Log a Jump Rope
  ...
  // ─── Calorie intensity per session (last 20 with calories) ──────────────
  const intensityData = sessionsWithCalories.slice(-20).map((s) => ({
    date: fmtDate(s.start_time),
    kpm: +(s.active_calories! / s.duration_minutes).toFixed(2),
  }))

  const intensityMax = intensityData.length
    ? Math.max(...intensityData.map((d) => d.kpm))
    : 0
          ...
          <BarChart data={weeklySessions} margin={{ top: 4, right: 4, left: -4, bottom: 0 }}>
            ...
            <Bar dataKey="count" fill={CYAN} radius={[3, 3, 0, 0]}>
              {weeklySessions.map((entry, i) => (
                <Cell
                  key={i}
                  fill={entry.count === weeklyMax && weeklyMax > 0 ? CYAN : CYAN_DIM}
                />
              ))}
            </Bar>
          </BarChart>
              ...
              <Bar dataKey="kpm" radius={[3, 3, 0, 0]}>
                {intensityData.map((entry, i) => (
                  <Cell
                    key={i}
                    fill={
                      entry.kpm === intensityMax && intensityMax > 0 ? CYAN : CYAN_DIM
                    }
                  />
                ))}
              </Bar>
