...
export async function GET() {
  ...
  const comparisons = Object.entries(METRIC_CONFIG).map(([metric, config]) => {
    ...
    if (metric === 'sleep_duration_minutes' && userValue) displayValue = Math.round(userValue / 6) / 10 // to hours
  ...
  return NextResponse.json({ comparisons, age, age_group: getAgeGroup(age), sex })
}
