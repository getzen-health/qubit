...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({ todayMeals: todayMeals ?? [], weekSummary, settings })
}

export async function POST(req: NextRequest) {
  // Rate limiting on POST
  const ip = req.headers.get('x-forwarded-for') ?? req.headers.get('x-real-ip') ?? 'unknown'
  const rl = await checkRateLimit(ip, 'healthData')
  if (!rl.allowed) {
  ...
  if (type === 'settings') {
    ...
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json({ settings: data, tdee: tdeeData, macros }, { status: 200 })
  }

  return NextResponse.json({ error: 'Invalid type. Must be "meal" or "settings".' }, { status: 400 })
}
