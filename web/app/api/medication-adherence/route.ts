...
export const GET = createSecureApiHandler(
  ...
  async (req, { user, supabase }) => {
    ...
    if (resource === 'medications') {
      ...
      return secureJsonResponse({ medications: data })
    }
    ...
    if (resource === 'logs') {
      ...
      if (error) return secureErrorResponse('Failed to fetch logs', 500)
    ...
    if (resource === 'compliance') {
      ...
      const { data, error } = await supabase
        ...
        .order('scheduled_time')
      if (error) return secureErrorResponse('Failed to fetch compliance', 500)
      return secureJsonResponse({ logs: data, days })
    }
    ...
    if (medsResult.error) return secureErrorResponse('Failed to fetch medications', 500)
    if (logsResult.error) return secureErrorResponse('Failed to fetch today\'s logs', 500)
    if (historyResult.error) return secureErrorResponse('Failed to fetch history', 500)
  ...
  async (req, { user, supabase }) => {
    ...
    if (resource === 'log') {
      ...
      if (!medication_id || !scheduled_time) {
        return secureErrorResponse('medication_id and scheduled_time are required', 400)
      }
      ...
      const { data: med } = await supabase
        ...
        .eq('user_id', user!.id)
        .single()
      if (!med) return secureErrorResponse('medication not found', 404)
      ...
      if (error) return secureErrorResponse('Failed to save log', 500)
    ...
    if (resource === 'medication') {
      ...
      if (!name || !frequency || !start_date) {
        return secureErrorResponse('name, frequency, and start_date are required', 400)
      }
    ...
    if (resource === 'deactivate') {
      ...
      if (!medication_id) {
        return secureErrorResponse('medication_id is required', 400)
      }
      ...
      if (error) return secureErrorResponse('Failed to deactivate medication', 500)
      return secureJsonResponse({ success: true })
    }

    return secureErrorResponse('unknown resource', 400)
  }
)
