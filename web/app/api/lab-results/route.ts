...
export async function GET() {
  ...
  const { data } = await supabase.from('lab_results').select('*').eq('user_id', user.id).order('lab_date', { ascending: false }).limit(200)
...
export async function DELETE(request: NextRequest) {
  ...
  await supabase.from('lab_results').delete().eq('id', id).eq('user_id', user.id)
