...
const TennisClient = dynamic(() => import('./tennis-client').then(m => ({ default: m.TennisClient })))
...
export default async function TennisPage() {
  ...
  const { data: sessions } = await supabase
    ...
    .eq('user_id', user.id)
    .in('workout_type', ['Tennis', 'TableTennis'])
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-md border-b border-border">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-bold text-text-primary">Tennis Analytics</h1>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 pb-24">
        <TennisClient sessions={sessions ?? []} />
      </main>
      <BottomNav />
    </div>
  )
