...
export function generateHealthReportPDF(data: HealthReportData): void {
  ...
  doc.setFontSize(20)
  doc.setFont('helvetica', 'bold')
  doc.text('⚡ GetZen Health Report', margin, 16)
  doc.setFontSize(9)
  doc.setFont('helvetica', 'normal')
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text('Generated by GetZen Health Tracker · kquarks.app', margin, 290)
    doc.text(`Page ${i} of ${pageCount}`, W - margin - 20, 290)
  }

  doc.save(`getzen-health-report-${new Date().toISOString().split('T')[0]}.pdf`)
}
