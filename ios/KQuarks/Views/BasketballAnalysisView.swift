...
// MARK: - BasketballAnalysisView

/// Tracks basketball sessions — pickup games, practice, and organized play.
/// Basketball is a high-intensity intermittent sport similar to soccer, with
/// short bursts at max effort and active recovery periods.
/// Covers HKWorkoutActivityType.basketball.
struct BasketballAnalysisView: View {
    ...
    private var summaryCard: some View {
        VStack(spacing: 12) {
            ...
            Divider()
            HStack(spacing: 0) {
                statCell(label: "Avg Duration", value: String(format: "%.0f min", avgDuration), color: .orange)
                Divider().frame(height: 36)
                statCell(label: "Avg kcal", value: String(format: "%.0f", avgKcal), color: .red)
                Divider().frame(height: 36)
                statCell(label: "Avg HR", value: avgHR > 0 ? "\(Int(avgHR)) bpm" : "—", color: .pink)
                Divider().frame(height: 36)
                statCell(label: "Peak HR", value: peakHR > 0 ? "\(Int(peakHR)) bpm" : "—", color: .red)
            }
        }
        .padding()
            ...
            VStack(spacing: 0) {
                ...
                ForEach(sessions.suffix(12).reversed()) { s in
                    ...
                    HStack {
                        ...
                        Text(String(format: "%.0f", s.kcal)).font(.caption.monospacedDigit()).foregroundStyle(.red).frame(width: 45, alignment: .trailing)
                        Text(String(format: "%.1f", s.kcalPerMin)).font(.caption.monospacedDigit()).foregroundStyle(.orange).frame(width: 45, alignment: .trailing)
                        Text(s.avgHR.map { String(format: "%.0f", $0) } ?? "—").font(.caption.monospacedDigit()).foregroundStyle(.pink).frame(width: 40, alignment: .trailing)
                    }
    ...
    private var emptyState: some View {
        VStack(spacing: 16) {
            ...
            Text("No Basketball Sessions")
                .font(.title3.bold())
            Text("Track basketball games and practice on Apple Watch using the Basketball workout type to see session history and heart rate data.")
    ...
    private func load() async {
        ...
        var allSessions: [Session] = workouts.map { w in
            Session(
...
#Preview { NavigationStack { BasketballAnalysisView() } }
