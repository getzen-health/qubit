...
// MARK: - MartialArtsView

/// Covers combat sports and martial arts workouts tracked on Apple Watch.
/// HKWorkoutActivityType: .kickboxing, .martialArts, .boxing, .wrestling,
/// .mixedMetabolicCardioTraining (used by some kickboxing apps).
/// These are high-intensity interval sports with one of the highest kcal/min
/// burn rates of any sport.
struct MartialArtsView: View {

    struct CombatSession: Identifiable {
        let id: UUID
        let date: Date
        ...
        var sportColor: Color {
            ...
            case .boxing:       return .pink
            case .wrestling:    return .blue
    ...
    @State private var sessions: [CombatSession] = []
    ...
    private let sports: [HKWorkoutActivityType] = [.kickboxing, .martialArts, .boxing, .wrestling]
    ...
    private var summaryCard: some View {
        VStack(spacing: 12) {
            ...
            Divider()
            HStack(spacing: 0) {
                statCell(label: "Avg Duration", value: String(format: "%.0f min", avgDuration), color: .red)
        ...
        return VStack(alignment: .leading, spacing: 8) {
            ...
            VStack(spacing: 0) {
                HStack {
                    ...
                    Text("Sport").font(.caption.weight(.semibold)).foregroundStyle(.secondary).frame(minWidth: 65, alignment: .leading)
                    Spacer()
                ...
                ForEach(sessions.suffix(12).reversed()) { s in
                    ...
                    HStack {
                        ...
                        Text(s.sportName).font(.caption).foregroundStyle(s.sportColor).frame(minWidth: 65, alignment: .leading)
                        Spacer()
    ...
    private var emptyState: some View {
        VStack(spacing: 16) {
            ...
            Text("No Combat Sports Sessions")
                .font(.title3.bold())
            Text("Track kickboxing, martial arts, boxing, or wrestling on Apple Watch using the appropriate workout type to see session history and performance data.")
    ...
    private func load() async {
        ...
        for sport in sports {
            ...
            for w in workouts {
                allSessions.append(CombatSession(
        ...
        for s in allSessions {
            ...
            cur.1 += 1; cur.2 += s.kcal
            bMap[key] = cur
        }
        weekBuckets = bMap.map { key, val in
            WeekBucket(id: key, weekStart: val.0, sessions: val.1, totalKcal: val.2)
        }.sorted { $0.weekStart < $1.weekStart }
    }
}

#Preview { NavigationStack { MartialArtsView() } }
