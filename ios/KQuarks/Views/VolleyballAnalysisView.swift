...
// MARK: - VolleyballAnalysisView

/// Analyzes volleyball workouts tracked via HKWorkoutActivityType.volleyball.
/// Covers both indoor and beach volleyball — key metrics are session frequency,
/// calorie burn, HR intensity, and weekly volume trends.
struct VolleyballAnalysisView: View {

    struct VBSession: Identifiable {
        let id: UUID
        let date: Date
    ...
    @State private var sessions: [VBSession] = []
    ...
    private var summaryCard: some View {
        VStack(spacing: 12) {
            ...
            Divider()
            HStack(spacing: 0) {
                statCell(label: "kcal/min", value: String(format: "%.1f", avgKcalPerMin), color: .yellow)
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            Chart {
                ForEach(sessions.suffix(20)) { s in
                    ...
                    .foregroundStyle(s.kcalPerMin >= 10 ? Color.orange.opacity(0.85) :
                                     s.kcalPerMin >= 7  ? Color.yellow.opacity(0.85) : Color.yellow.opacity(0.5))
                    .cornerRadius(2)
                }
                if avgKcalPerMin > 0 {
            ...
            VStack(spacing: 0) {
                ...
                ForEach(sessions.suffix(12).reversed()) { s in
                    ...
                    HStack {
                        ...
                        Text(String(format: "%.0f", s.kcal)).font(.caption.monospacedDigit()).foregroundStyle(.yellow).frame(width: 45, alignment: .trailing)
    ...
    private var emptyState: some View {
        VStack(spacing: 16) {
            ...
            Text("No Volleyball Sessions")
                .font(.title3.bold())
            Text("Track indoor or beach volleyball on Apple Watch using the Volleyball workout type to see your session history, calorie burn, and heart rate data here.")
                .font(.subheadline).foregroundStyle(.secondary)
...
#Preview { NavigationStack { VolleyballAnalysisView() } }
