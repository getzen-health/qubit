...
/// HIIT-specific pattern analysis: day-of-week distribution, time-of-day
/// preference, weekly volume, monthly sessions, and duration trend.
struct HIITPatternView: View {
    ...
    @State private var longestMins: Double = 0
    ...
    private var summaryCards: some View {
        LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 10) {
            statCard(label: "Sessions (1yr)", value: "\(totalSessions)", sub: String(format: "%.1f/week avg", avgPerWeek), color: .yellow)
            statCard(label: "Avg Duration", value: "\(Int(avgDurationMins)) min", sub: "Longest: \(Int(longestMins))m", color: .orange)
            statCard(label: "Total Hours", value: String(format: "%.1f h", totalHours), sub: "past year", color: .red)
    ...
    private var dowChart: some View {
        ...
        return VStack(alignment: .leading, spacing: 10) {
            ...
            Text("Which days you do HIIT").font(.caption).foregroundStyle(.secondary)
            Chart {
                ForEach(dayBuckets) { b in
                    BarMark(x: .value("Day", b.label), y: .value("Sessions", b.sessions))
                        .foregroundStyle(b.sessions == maxSessions ? Color.yellow : Color.yellow.opacity(0.4))
                        .cornerRadius(4)
            ...
            if !busiestDay.isEmpty {
                Label("Peak day: \(busiestDay)", systemImage: "bolt.fill").font(.caption2).foregroundStyle(.yellow)
            }
        }
        .padding()
        .background(Color(.systemBackground))
    ...
    private var timeOfDaySection: some View {
        ...
        return VStack(alignment: .leading, spacing: 10) {
            ...
            Text("When you train").font(.caption).foregroundStyle(.secondary)
            VStack(spacing: 8) {
                timeBar(label: "🌅 Morning", sessions: morningTotal, total: total, color: .yellow)
                timeBar(label: "☀️ Afternoon", sessions: afternoonTotal, total: total, color: .orange)
                timeBar(label: "🌙 Evening", sessions: eveningTotal, total: total, color: .indigo)
            }
            ...
            Chart {
                ForEach(weekVolumes) { w in
                    BarMark(x: .value("Week", w.id), y: .value("Mins", w.mins))
                        .foregroundStyle(w.mins >= maxMins * 0.85 ? Color.yellow : Color.orange.opacity(0.5))
                        .cornerRadius(4)
                }
                if avgMins > 0 {
            ...
            Chart {
                ForEach(monthStats) { m in
                    BarMark(x: .value("Month", m.label), y: .value("Sessions", m.sessions))
                        .foregroundStyle(Color.yellow.opacity(0.7)).cornerRadius(4)
                }
            }
            .chartXAxis { AxisMarks(values: .automatic) { _ in AxisValueLabel().font(.caption2) } }
            .chartYAxis {
    ...
    private var guidelinesCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            Label("HIIT Guidelines", systemImage: "bolt.heart.fill")
                .font(.subheadline.weight(.semibold)).foregroundStyle(.yellow)
            let tips: [(String, String)] = [
                ("Frequency", "2–3 HIIT sessions/week max due to CNS demand"),
                ("Recovery", "Allow 48h between sessions; HRV drop >15% signals incomplete recovery"),
                ("Duration", "True HIIT is 15–30 min; longer sessions drift into moderate cardio"),
                ("Intensity", "Work at 85–95% max HR; rest until HR drops below 65%"),
            ]
            ForEach(tips, id: \.0) { title, desc in
                HStack(alignment: .top, spacing: 8) {
                    Circle().fill(Color.yellow.opacity(0.6)).frame(width: 6, height: 6).padding(.top, 5)
                    VStack(alignment: .leading, spacing: 1) {
                        Text(title).font(.caption.weight(.semibold)).foregroundStyle(.yellow)
    ...
    private var emptyState: some View {
        VStack(spacing: 16) {
            Image(systemName: "bolt.heart.fill").font(.system(size: 60)).foregroundStyle(.yellow.opacity(0.4))
            Text("Not Enough Data").font(.title3.bold())
            Text("Log at least 3 HIIT sessions to see your patterns.")
    ...
    private func load() async {
        ...
        let sessions = allWorkouts.filter {
            ($0.workoutActivityType == .highIntensityIntervalTraining) && $0.duration > 180
        }
        ...
        await MainActor.run {
            ...
            self.longestMins = longestSessionMins
...
#Preview {
    NavigationStack { HIITPatternView() }
}
