-- Habit tracker: habits, logs, and user achievements
-- Behavioral science backed: Lally 2010, Fogg 2020, Duhigg 2012
...
create policy "Users manage own habit logs" on public.habit_logs
  for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
create index idx_habit_logs_user_date on public.habit_logs(user_id, completed_at desc);
